		...
		<view class="content">
			<view :style='{"minHeight":"100vh","alignContent":"flex-start","padding":"20rpx 20rpx 240rpx","alignItems":"flex-start","flexWrap":"wrap","background":"linear-gradient(135.00deg, rgb(206, 209, 242),rgb(228, 185, 225) 100%)","display":"flex","width":"100%","height":"auto"}'>
						...
						<block v-for="(product,index) in list" :key="index">
							<view v-if="index%2==0" class="list-item" :style='{"boxShadow":"inset 0px 2rpx 8rpx 0px rgb(131, 122, 161)","padding":"20rpx 0 20rpx 20rpx","margin":"0 0 20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
								...
								<view class="list-item-body" :style='{"padding":"0","margin":"0","flexWrap":"wrap","display":"flex","width":"calc(100% - 200rpx)","justifyContent":"space-between","height":"auto"}' @tap="onDetailTap(product)">
									...
									<view :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#000"}' class="title">医生姓名:{{product.yishengxingming}}</view>
									<view :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#000"}' class="title">患者姓名:{{product.huanzhexingming}}</view>
									...
									<view :style='{"padding":"0 20rpx","order":"2"}'>
										...
										<text :style='{"color":"#8FA771","lineHeight":"40rpx","fontSize":"24rpx"}'>{{product.yishengzhanghao}}</text>
									</view>
								</view>
								<!-- #ifdef MP-WEIXIN -->
								<view :style='{"width":"100%","padding":"8rpx 20rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
									<view :style='{"border":"2rpx solid #BCFFFD","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zhenliaoxinxi','修改')) || (!userid && isAuthFront('zhenliaoxinxi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
									...
									<view :style='{"border":"2rpx solid #E4AED7","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zhenliaoxinxi','删除')) || (!userid && isAuthFront('zhenliaoxinxi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
								...
								<!-- #ifndef MP-WEIXIN -->
								<view :style='{"width":"100%","padding":"8rpx 20rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
									<view :style='{"border":"2rpx solid #BCFFFD","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zhenliaoxinxi','修改')) || (!userid && isAuthFront('zhenliaoxinxi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
										<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"40rpx","color":"#E4B9E1","display":"inline-block"}' class="cuIcon-edit"></text>
										<text :style='{"fontSize":"24rpx","lineHeight":"40rpx","color":"#E4B9E1","display":"inline-block"}'>修改</text>
									</view>
									<view :style='{"border":"2rpx solid #E4AED7","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zhenliaoxinxi','删除')) || (!userid && isAuthFront('zhenliaoxinxi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
										<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"40rpx","color":"#E4AED7","display":"inline-block"}' class="cuIcon-delete"></text>
										<text :style='{"fontSize":"24rpx","lineHeight":"40rpx","color":"#E4AED7","display":"inline-block"}'>删除</text>
									</view>
								</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					...
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"26rpx","color":"#858585"}'>患者姓名</view>
						<input :style='{"border":"2rpx solid rgb(214, 214, 214)","width":"90%","padding":"0 20rpx 0 20rpx","borderRadius":"4rpx","background":"#F8F8F8","height":"64rpx"}' placeholder="请输入患者姓名" v-model="searchForm.huanzhexingming">
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zhenliaoxinxi`, params);
                } else {
                    res = await this.$api.list(`zhenliaoxinxi`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhenliaoxinxi', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zhenliaoxinxi`, searchForm);
                } else {
                    res = await this.$api.list(`zhenliaoxinxi`, searchForm);
                }
				// 如果是第一页数据置空
