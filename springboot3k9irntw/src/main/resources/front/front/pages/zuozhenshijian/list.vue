			...
			<view :style='{"minHeight":"100vh","alignContent":"flex-start","padding":"20rpx 20rpx 240rpx","alignItems":"flex-start","flexWrap":"wrap","background":"linear-gradient(135.00deg, rgb(206, 209, 242),rgb(228, 185, 225) 100%)","display":"flex","width":"100%","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","boxShadow":"inset 0px 2rpx 8rpx 0px rgb(131, 122, 161)","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.zuozhenshijian" type="text" placeholder="坐诊时间" ></input>
					</view>
						...
						<block v-for="(product,index) in list" :key="index">
							<view v-if="index%2==0" class="list-item" :style='{"boxShadow":"inset 0px 2rpx 8rpx 0px rgb(131, 122, 161)","padding":"20rpx 0 20rpx 20rpx","margin":"0 0 20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
								...
								<view :style='{"width":"100%","padding":"8rpx 20rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
									<view :style='{"border":"2rpx solid #BCFFFD","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zuozhenshijian','修改')) || (!userid && isAuthFront('zuozhenshijian','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
									...
									<view :style='{"border":"2rpx solid #E4AED7","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zuozhenshijian','删除')) || (!userid && isAuthFront('zuozhenshijian','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
								...
								<!-- #ifndef MP-WEIXIN -->
								<view :style='{"width":"100%","padding":"8rpx 20rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
									<view :style='{"border":"2rpx solid #BCFFFD","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zuozhenshijian','修改')) || (!userid && isAuthFront('zuozhenshijian','修改'))" @tap.stop.proevent="onUpdateTap(product)">
										<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"40rpx","color":"#E4B9E1","display":"inline-block"}' class="cuIcon-edit"></text>
										<text :style='{"fontSize":"24rpx","lineHeight":"40rpx","color":"#E4B9E1","display":"inline-block"}'>修改</text>
									</view>
									<view :style='{"border":"2rpx solid #E4AED7","padding":"0 16rpx","background":"#fff","display":"flex"}' v-if="(userid && isAuth('zuozhenshijian','删除')) || (!userid && isAuthFront('zuozhenshijian','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
										<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"40rpx","color":"#E4AED7","display":"inline-block"}' class="cuIcon-delete"></text>
										<text :style='{"fontSize":"24rpx","lineHeight":"40rpx","color":"#E4AED7","display":"inline-block"}'>删除</text>
									</view>
								</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zuozhenshijian){
					params['zuozhenshijian'] = '%' + this.searchForm.zuozhenshijian + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zuozhenshijian`, params);
                } else {
                    res = await this.$api.list(`zuozhenshijian`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zuozhenshijian', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.zuozhenshijian){
					searchForm['zuozhenshijian'] = '%' + this.searchForm.zuozhenshijian + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zuozhenshijian`, searchForm);
                } else {
                    res = await this.$api.list(`zuozhenshijian`, searchForm);
                }
				// 如果是第一页数据置空
