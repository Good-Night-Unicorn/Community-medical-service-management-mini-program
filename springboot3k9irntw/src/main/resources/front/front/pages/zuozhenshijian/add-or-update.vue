	...
	<view :style='{"minHeight":"100%","padding":"20rpx","alignItems":"flex-start","background":"linear-gradient(135.00deg, rgb(206, 209, 242),rgb(228, 185, 225) 100%)","display":"flex","width":"100%","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","borderRadius":"20rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"12rpx 0","boxShadow":"0px 8rpx 8rpx 0px rgb(234, 236, 255)","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0","padding":"0px 20rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"70rpx"}' :disabled="ro.zuozhenshijian" v-model="ruleForm.zuozhenshijian" placeholder="坐诊时间"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zuozhenshijian`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`zuozhenshijian`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`zuozhenshijian`, this.ruleForm);
						}else{
							oet = await this.$api.add(`zuozhenshijian`, this.ruleForm);
						}
