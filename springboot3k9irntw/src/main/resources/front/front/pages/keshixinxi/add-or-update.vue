...
<view class="content">
	<view :style='{"minHeight":"100%","padding":"20rpx","alignItems":"flex-start","background":"linear-gradient(135.00deg, rgb(206, 209, 242),rgb(228, 185, 225) 100%)","display":"flex","width":"100%","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","borderRadius":"20rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","boxShadow":"0px 8rpx 8rpx 0px rgb(234, 236, 255)","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class=" select">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"70rpx","fontSize":"28rpx","color":"#000","textAlign":"right"}' class="title">坐诊时间</view>
				<picker :disabled="ro.zuozhenshijian" :style='{"width":"100%","flex":"1","height":"auto"}' @change="zuozhenshijianChange" :value="zuozhenshijianIndex" :range="zuozhenshijianOptions">
					<view :style='{"width":"100%","padding":"0 20rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#D4C1D6"}' class="uni-input">{{ruleForm.zuozhenshijian?ruleForm.zuozhenshijian:"请选择坐诊时间"}}</view>
				</picker>
			</view>
			<view :style='{"padding":"12rpx 0","boxShadow":"0px 8rpx 8rpx 0px rgb(234, 236, 255)","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
			...
			<view :style='{"padding":"12rpx 0","boxShadow":"0px 8rpx 8rpx 0px rgb(234, 236, 255)","margin":"0 0 24rpx 0","borderColor":"#ccc","flexWrap":"wrap","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"70rpx","fontSize":"28rpx","color":"#000","textAlign":"right"}' class="title">科室介绍</view>
				<xia-editor ref="keshijieshao" :style='{"minHeight":"300rpx","border":"0px solid #efefef","width":"calc(100% - 160rpx)","padding":"20rpx","height":"auto"}' v-model="ruleForm.keshijieshao" placeholder="科室介绍" @editorChange="keshijieshaoChange"></xia-editor>
			</view>
	...
	export default {
		data() {
			return {
				...
				keshifenleiOptions: [],
				keshifenleiIndex: 0,
				zuozhenshijianOptions: [],
				zuozhenshijianIndex: 0,
				// 登录用户信息
				user: {},
				ro:{
		...
		async onLoad(options) {
			...
			// 下拉框
			res = await this.$api.option(`zuozhenshijian`,`zuozhenshijian`,{});
			this.zuozhenshijianOptions = res.data;
			this.zuozhenshijianOptions.unshift("请选择坐诊时间");
		...
		methods: {
			keshijieshaoChange(e) {
				this.ruleForm.keshijieshao = e
			},
			styleChange() {
				this.$nextTick(()=>{
			...
			// 下拉变化
			zuozhenshijianChange(e) {
				this.zuozhenshijianIndex = e.target.value
				this.ruleForm.zuozhenshijian = this.zuozhenshijianOptions[this.zuozhenshijianIndex]
			},
			...
			async onSubmitTap() {
				...
				//跨表计算
						...
						if(this.ruleForm.id){
							...
							oet = await this.$api.add(`keshixinxi`, this.ruleForm);
						}
