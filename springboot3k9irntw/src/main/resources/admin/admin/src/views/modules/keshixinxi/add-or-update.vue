	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="坐诊时间" prop="zuozhenshijian" >
					<el-select :disabled="ro.zuozhenshijian" v-model="ruleForm.zuozhenshijian" placeholder="请选择坐诊时间" >
						<el-option
							v-for="(item,index) in zuozhenshijianOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
			...
			<el-form-item v-else-if="ruleForm.keshijieshao" label="科室介绍" prop="keshijieshao" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.keshijieshao"></span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.$http({
					url: `option/zuozhenshijian/zuozhenshijian`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.zuozhenshijianOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.keshijieshao = this.ruleForm.keshijieshao.replace(reg,'../../../springboot3k9irntw/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
